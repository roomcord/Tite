...
local function getWorkRoots()
	local roots, seen = {}, {}
	for _, child in ipairs(PlayerGui:GetChildren()) do
		if not seen[child] and child.Name:lower():find("work", 1, true) then
			seen[child] = true
			table.insert(roots, child)
		end
	end
	if #roots == 0 then
		local wg = PlayerGui:FindFirstChild("WorkGui")
		if wg and not seen[wg] then table.insert(roots, wg) end
	end
	return roots
end

local function findAnswerButton(ansText, solved, qText)
	local roots = getWorkRoots()
	...
	for _, root in ipairs(roots) do
		for _, obj in ipairs(root:GetDescendants()) do
			if (obj:IsA("TextButton") or obj:IsA("ImageButton")) and isVisible(obj) then
				local matchObj = nil
				if textMatches(getGuiText(obj), ansText, solved) then
					matchObj = obj
				else
					for _, c in ipairs(obj:GetDescendants()) do
				...
				if matchObj then
					...
					if dk == "" then dk = tostring(matchObj) end
					if not seen[dk] then
						seen[dk] = true
...
local function doAnswer(question, answers, sessionId, attempt, token)
	...
	if answering then
		task.delay(0.15, function() doAnswer(question, answers, sessionId, attempt, token) end)
		return
	end
	answering = true
	task.spawn(function()
		local function finish(retryNow, statusMsg)
			answering = false
			if statusMsg then setStatus(statusMsg) end
			if retryNow and attempt < MAX_RETRIES and active and token == activeToken then
				task.delay(RETRY_DELAY, function() doAnswer(question, answers, sessionId, attempt+1, token) end)
			end
		end

		if not active or token ~= activeToken then finish(false) return end

		local solved = solveQ(question)
		if not solved then finish(false, "cant solve") return end
		...
		task.wait(ANS_DELAY_MIN + math.random() * (ANS_DELAY_MAX - ANS_DELAY_MIN))
		if not active or token ~= activeToken then finish(false) return end

		local btn, candidates = findAnswerButton(ansText, solved, question)
		if not btn or #candidates == 0 then
			finish(true, "retrying (" .. attempt .. ")")
			return
		end
		...
		for _, cand in ipairs(candidates) do
			...
			if r == true or tostring(r):lower() == "success" then result = r break end
			task.wait(0.1)
		end
