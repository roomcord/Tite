...
local RueyGui, StatusLabel, QLabel, PLabel, MoneyLabel, EarnedLabel, ToggleBtn
...
local function getAnswerRoots()
	local roots = {}
	local seen = {}
	local function add(r)
		if r and not seen[r] then seen[r] = true table.insert(roots, r) end
	end
	add(PlayerGui:FindFirstChild("WorkGui"))
	for _, sg in ipairs(PlayerGui:GetChildren()) do
		if sg:IsA("ScreenGui") and not sg:IsDescendantOf(RueyGui) and sg ~= RueyGui then
			add(sg)
		end
	end
	for _, obj in ipairs(workspace:GetDescendants()) do
...
local function isOfficeAnswerCandidate(obj)
	...
	while cur do
		...
		if cur:IsA("SurfaceGui") or cur:IsA("BillboardGui") then return true end
		if cur:IsA("ScreenGui") then
			return name ~= "rueyhubofficefarm" and name ~= "robloxgui" and name ~= "coregui"
		end
...
local function findAnswerButton(answerText, solvedValue, questionText)
	...
	local candidates = {}
	local seenKeys = {}
	for _, root in ipairs(getAnswerRoots()) do
		for _, obj in ipairs(root:GetDescendants()) do
...
local function hookOfficeRemotes()
	if questionConnection then return end
	...
	while toggled do
		...
		while toggled do
			if not active then
				setStatus("paused")
				releaseSprint()
				while not active do task.wait(0.2) end
				setStatus("resumed")
			end
			antiAfkTick()
...
task.spawn(function()
	while RueyGui.Parent do
		...
		local earned = (toggled and sessionStartMoney) and math.max(0, money - sessionStartMoney) or 0
		if MoneyLabel then MoneyLabel.Text = "Rp. " .. formatNumber(money) end
		if EarnedLabel and toggled then EarnedLabel.Text = "Rp. " .. formatNumber(earned) end
	end
end)
...
print("[ruey hub] office farm loaded — press ON to start")
